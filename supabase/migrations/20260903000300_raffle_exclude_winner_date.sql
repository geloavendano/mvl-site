-- Allow admins to include prior winners generally while still excluding
-- anyone whose prize record was created on one specific Manila-calendar date.
-- Earlier draw RPCs remain available for cached admin clients.
--
-- Apply individually, NOT with `supabase db push`.

create or replace function public.mvl_admin_draw_raffle_winner_with_date_filters(
  p_start_date date,
  p_end_date date,
  ...
  p_excluded_winner_date date default null
) returns jsonb
language plpgsql
volatile
security definer
...
declare
  ...
  v_excluded_winner_date date := p_excluded_winner_date;
begin
  ...
  return (
    with eligible as (
      ...
      where (v_include_previous_winners or not e.is_blacklisted)
        ...
        and (
          v_excluded_winner_date is null
          or not exists (
            select 1
            from mvl.raffle_blacklist b
            where b.player_id = e.player_id
              and (b.created_at at time zone 'Asia/Manila')::date = v_excluded_winner_date
          )
        )
    ), chosen as (
    ...
    select jsonb_build_object(
      ...
      'excludedWinnerDate', v_excluded_winner_date,
...
revoke all on function public.mvl_admin_draw_raffle_winner_with_date_filters(date, date, boolean, boolean, text[], date) from public;
revoke all on function public.mvl_admin_draw_raffle_winner_with_date_filters(date, date, boolean, boolean, text[], date) from anon;
grant execute on function public.mvl_admin_draw_raffle_winner_with_date_filters(date, date, boolean, boolean, text[], date) to authenticated;
