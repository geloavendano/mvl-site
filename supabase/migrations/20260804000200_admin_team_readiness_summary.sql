-- Admin-only readiness totals for every tournament team on one Manila date.

create or replace function public.mvl_admin_get_readiness_summary(
...
revoke all on function public.mvl_admin_get_readiness_summary(date) from public;
grant execute on function public.mvl_admin_get_readiness_summary(date) to authenticated;
