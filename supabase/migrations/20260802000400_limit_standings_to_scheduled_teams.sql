-- Keeps waiver-only/test teams out of standings.
--
-- Apply with:
--   supabase db query --linked --file supabase/migrations/20260802000400_limit_standings_to_scheduled_teams.sql

create or replace function public.mvl_get_standings()
returns table (
  rank integer,
  team_id text,
  team_name text,
...
with scheduled_teams as (
  ...
  from mvl.teams t
  join mvl.games g on t.id in (g.team_a_id, g.team_b_id)
),
team_match_stats as (
  select
    t.id as team_id,
  ...
  from mvl.games g
  join scheduled_teams t on t.id in (g.team_a_id, g.team_b_id)
