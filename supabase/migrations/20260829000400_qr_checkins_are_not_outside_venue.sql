-- A staff QR scan never records a location: mvl_qr_checkin runs no geofence
-- check, because the booth is at the venue by definition. It stores
-- inside_radius = NULL, and coalesce(..., false) downstream read that as
-- "failed the check" — so every booth scan showed in the admin console as
-- "Outside venue". On 2026-08-29 that was 22 of 81 valid check-ins.
--
-- Treat NULL on a qr row as inside. A self check-in that genuinely fell
-- outside still stores false and is still flagged.
--
-- Apply individually, NOT with `supabase db push`.

CREATE OR REPLACE FUNCTION public.mvl_admin_get_player_readiness(p_team_id text DEFAULT NULL::text, p_day date DEFAULT NULL::date)
 RETURNS jsonb
 LANGUAGE plpgsql
  ...
  return jsonb_build_object(
    ...
    'players', coalesce((
      ...
      from (
        ...
        left join lateral (
          select
            bool_or(coalesce(r.inside_radius, r.method = 'qr')) as inside_radius,
            max(r.created_at) filter (where coalesce(r.inside_radius, r.method = 'qr')) as checked_in_at,
