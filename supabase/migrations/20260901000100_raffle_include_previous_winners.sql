-- Allow admins to optionally include prior raffle winners in a new draw.
-- The existing draw RPC remains available for older cached admin clients.
-- Apply individually, NOT with `supabase db push`.

create or replace function public.mvl_admin_draw_raffle_winner_with_options(
  p_start_date date,
  p_end_date date,
...
revoke all on function public.mvl_admin_draw_raffle_winner_with_options(date, date, boolean, boolean) from public;
grant execute on function public.mvl_admin_draw_raffle_winner_with_options(date, date, boolean, boolean) to authenticated;
