...
const teamOptions = (selected, placeholder) => [
  placeholder ? `<option value="__placeholder__" selected>${placeholder} (placeholder)</option>` : '',
  ...window.MVL_DATA.teams.map((team) =>
...
const render = () => {
  document.getElementById('adminGameList').innerHTML = data.publicData.games.map((g) => `<form class="admin-panel admin-game admin-form" data-id="${g.id}">
    ...
    <div class="admin-fields admin-fields--three"><label class="field"><span>Winner</span><select name="winner" required ${g.teamALabel || g.teamBLabel ? 'disabled' : ''}><option value="">Choose</option><option value="${g.teamA}" ${g.winner === g.teamA ? 'selected' : ''}>${gameTeamName(g, 'A')}</option><option value="${g.teamB}" ${g.winner === g.teamB ? 'selected' : ''}>${gameTeamName(g, 'B')}</option></select></label><label class="field"><span>YouTube link or video ID</span><input name="youtubeId" value="${g.youtubeId || ''}" placeholder="https://youtu.be/... or video ID"><small>The video ID is the code after youtu.be/ or v=; a full link also works.</small></label><label class="field"><span>Duration H:MM:SS</span><input name="duration" value="${g.duration || ''}"></label></div>
    <div class="admin-score-heading"><span>Set</span><strong>${gameTeamName(g, 'A')}</strong><strong>${gameTeamName(g, 'B')}</strong></div>
    <div class="admin-sets">${[0,1,2,3,4].map((i) => `<label><span>Set ${i+1}</span><input name="a${i}" aria-label="${gameTeamName(g, 'A')} set ${i+1}" type="number" min="0" value="${g.sets[i]?.a ?? ''}" ${g.teamALabel || g.teamBLabel ? 'disabled' : ''}><input name="b${i}" aria-label="${gameTeamName(g, 'B')} set ${i+1}" type="number" min="0" value="${g.sets[i]?.b ?? ''}" ${g.teamALabel || g.teamBLabel ? 'disabled' : ''}></label>`).join('')}</div>
    <button class="cta cta--primary" ${g.teamALabel || g.teamBLabel ? 'disabled title="Assign the actual teams before entering a result."' : ''}>Save result & video</button><p class="form-status"></p></form>`).join('');
};
const show = async () => {
  try {
