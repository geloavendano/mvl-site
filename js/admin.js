...
const render = () => {
  document.getElementById('adminGameList').innerHTML = data.publicData.games.map((g) => `<form class="admin-panel admin-game admin-form" data-id="${g.id}">
    ...
    <div class="admin-fields admin-fields--schedule"><label class="field"><span>Match day number</span><input name="day" type="number" min="1" value="${g.day}" required></label><label class="field"><span>Date & time (Philippines)</span><input name="startsAt" type="datetime-local" value="${manilaDateTime(g.startsAt)}" required></label><button class="cta cta--secondary" type="button" data-save-schedule>Save schedule</button></div>
...
adminGameList.addEventListener('click', async (e) => { const form=e.target.closest('[data-id]'); if(!form)return; const s=form.querySelector('.form-status'); if(e.target.closest('[data-save-schedule]')){status(s,'Saving schedule…');try{const local=form.elements.startsAt.value;await rpc('mvl_admin_update_game_schedule',{p_game_id:form.dataset.id,p_day:+form.elements.day.value,p_starts_at:new Date(`${local}:00+08:00`).toISOString()});status(s,'Schedule updated.','success');}catch(err){status(s,err.message,'error');}return;} const btn=e.target.closest('[data-reset]'); if(!btn)return; status(s,'Resetting…'); try{await rpc('mvl_admin_reset_game',{p_game_id:form.dataset.id});form.elements.winner.value='';form.querySelectorAll('.admin-sets input').forEach((i)=>i.value='');status(s,'Reset to pending.','success');}catch(err){status(s,err.message,'error');}});
